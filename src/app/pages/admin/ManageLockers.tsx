...
import {
  ...
  CheckCircle2, BookMarked, ShieldAlert, X, Trash2,
} from 'lucide-react';
...
// ─── Delete Locker Modal ───────────────────────────────────────────────────────
function DeleteLockerModal({ locker, onClose }: { locker: Locker; onClose: () => void }) {
  const { deleteLocker } = useApp();
  ...
  const handleDelete = async () => {
    setLoading(true);
    setError('');
    const result = await deleteLocker(locker.id);
    setLoading(false);
    if (!result.success) {
      setError(result.message);
    } else {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-800" style={{ fontWeight: 600 }}>Delete Locker</span>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500">
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="p-6 space-y-4">
          {done ? (
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" />
              <p className="text-sm text-emerald-600">Locker deleted successfully.</p>
            </div>
          ) : (
            <>
              <div className="flex gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                ...
                <p className="text-sm text-red-700">
                  Are you sure you want to permanently delete locker <strong>{locker.number}</strong>?
                  This action cannot be undone. Only <strong>available</strong> lockers can be deleted.
                </p>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400 flex items-center gap-1">
                    <MapPin className="w-3 h-3" />{locker.location}
                  </div>
                </div>
                <span className={`ml-auto text-xs px-2 py-0.5 rounded-full ${
                  locker.status === 'available' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                }`} style={{ fontWeight: 600 }}>
                  {locker.status}
                </span>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  ...
                  {loading ? 'Deleting…' : 'Delete Locker'}
                </button>
              </div>
            </>
          )}
...
// ─── Locker Row ────────────────────────────────────────────────────────────────
function LockerRow({ locker, onChangeStatus, onDelete }: { locker: Locker; onChangeStatus: (l: Locker) => void; onDelete: (l: Locker) => void }) {
    ...
    <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
      ...
      <div className="flex gap-2 sm:flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded-lg text-xs transition-colors"
          style={{ fontWeight: 600 }}
        >
...
export function ManageLockers() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="max-h-[560px] overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            filtered.map(l => (
              <LockerRow key={l.id} locker={l} onChangeStatus={setEditLocker} onDelete={setDeleteTarget} />
            ))
          )}
        </div>
      </div>
      ...
      {deleteTarget && <DeleteLockerModal locker={deleteTarget} onClose={() => setDeleteTarget(null)} />}
    </div>
  );
}